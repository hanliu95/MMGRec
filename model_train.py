...
class EncoderLayer(nn.Module):
    def __init__(self):
        ...
        self.sca = nn.Linear(d_model, 1, bias=False)
        self.fc = nn.Linear(n_heads * d_v, d_model, bias=False)
        self.pos_ffn = PoswiseFeedForwardNet()
        self.dropout = nn.Dropout(p=0.2)
    ...
    def forward(self, input, attn_mask, input_u):
        ...
        S = self.sca(input_u).repeat(1, input.size(1), d_k)
        u_Q = torch.mul(self.U_Q(input), S).view(batch_size, -1, 1, d_k).transpose(1, 2)
        u_K = torch.mul(self.U_K(input), S).view(batch_size, -1, 1, d_k).transpose(1, 2)
        u_V = torch.mul(self.U_V(input), S).view(batch_size, -1, 1, d_v).transpose(1, 2)
